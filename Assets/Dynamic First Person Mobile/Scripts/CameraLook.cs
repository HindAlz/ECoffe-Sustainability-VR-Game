	...
	public class CameraLook : MonoBehaviour {	
        ...
        private void Start()
        {
            ...
            if (Camera.main != null)
                ...
                Debug.LogError($"Can't find any main camera in scene!\n(Set your camera tag as MainCamera)", this);

            // Check if there is an EventSystem, and if not, assign it to null
            if (EventSystem.current != null)
                m_EventStytem = EventSystem.current;
            else
            {
                Debug.LogWarning("No Event System found in the scene. Touch input may not be properly detected for UI elements.");
                m_EventStytem = null;  // Set it to null if no EventSystem is found
            }
